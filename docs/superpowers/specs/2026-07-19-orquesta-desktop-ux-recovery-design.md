...
Orquesta管理係は必須roleにしない。canonical stateに存在するprojectだけ表示する。存在する場合も、user直属のmanagement satelliteとして扱う。

user-liaison、vision-curator、error-conciergeはuser-support laneへ置く。production specialistはorchestrator配下に置く。
