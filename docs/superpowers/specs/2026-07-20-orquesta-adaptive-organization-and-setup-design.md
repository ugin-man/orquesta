...
同役割メンバーの自動追加は、プロジェクト設定が `auto_within_budget` の場合だけ許可する。既定値は次の通りとする。

```json
{
  "same_role_scaling": "auto_within_budget",
  "max_active_agents": 12,
  "max_agents_per_role_per_line": 3,
  "require_no_file_ownership_conflict": true
}
```

上限を超える場合は提案に切り替える。追加理由と作成した個体は必ずユーザー判断履歴へ記録する。
...
- 組織判断だけの多段レビューを作らない
- 新しいスレッドは承認後にまとめて provisioning する
...
- ユーザー承認待ちのラインや役割は、実在する組織として表示しない。提案として表示する
...
- 運用中は、上限内の同役割追加だけ自動で行える
- 新役割、新ライン、専任リーダー、恒久移籍はユーザー承認を必要とする
- ライン間の一時応援は作らない
