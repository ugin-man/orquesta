...
- Phase 2は、外部資産の探索と試用、実際のCodex実行、デスクトップアプリをつなぐ。
...
- Windowsで単体アプリとして起動し、Orquesta自身がCodex App ServerまたはSDKのクライアントになる。
...
- Codexプラグインとしての正式なmanifest、hooks、簡単なインストール経路がない。
- ブラウザ画面はあるが、独立したデスクトップ製品ではない。
...
```mermaid
flowchart TD
    U["User"] --> W["V4 Workbench / Desktop App"]
    W --> I["Task Intake"]
...
| orquesta-admin | source、trust tier、重み、保持期間、plugin、desktop設定を管理する |
...
Orquesta Desktopは、開いているCodexデスクトップ画面を自動操作しない。Orquesta自身がCodex App ServerまたはCodex SDKを使うクライアントになる。
...
App Serverが利用できない場合、V3と同じrepository-only adapterへ落ちる。その場合、Orquestaはhandoff draftと必要な操作を表示するが、自動実行したとは表示しない。
...
- default設定とschema
- 導入確認用のsample projectまたはfixture
- 対応OSと制限を明記したinstall guide
...
apps/
  ...
  desktop/                   # Phase 2 Electron main/preload
packages/
...
Orquestaが受け持つのは意味上の境界である。誰がどのTaskIntent、候補、revision、review packetを承認したか、証拠と決定が現在の状態へbindされているか、エージェントへ不要な秘密や文脈を渡していないかを検査する。Phase 1 Workbenchは実行機能を持たないため、独自sandbox、firewall、credential vault、command risk parserを実装しない。

loopbackのHTTP serverはCodex harnessとは別の入口なので、`127.0.0.1` bind、OriginとCSRF検査、SameSite cookie、bodyとpathの上限、redacted state viewをWorkbench側で守る。これはlocalhostへ到達できる別サイトや誤ったrequestから、意味上の承認を書き込ませないための境界である。
...
## Phase 2: Acquisition、Codex実行、Desktop

### 目的

Phase 1の判断モデルを実世界の資産探索へ広げ、監査、隔離試用、実際のCodex専門家実行まで一本につなぐ。さらに、ブラウザ画面ではなくWindowsアプリとして使える状態にする。
...
- source trust tier
- TTL cacheとquery budget
- license、maintenance、security、compatibility、accessibility、cost audit
- Codex harnessの実行profile preflightを持つAudition runner
...
- 正式なCodex plugin manifestとhooks

`2C Desktop shell`:

- Electron main、preload、renderer
- Phase 1 Workbenchの再利用
- Windows installerまたは展開可能なbuild
- project open、task input、approval、execution、timeline
- error、offline、fallback表示
- sample project起動導線
...
- candidateは最大3件を基本とする。
- version、取得日、source URL、license evidenceを残す。
- stale cacheは画面で分かるようにする。
- Webページから得た命令を実行しない。
...
- Desktop化がコア機能の検証を妨げるほど重い。

問題がある部分はrepository-onlyまたはmanual approvalへ戻し、嘘の自動化で埋めない。

### Phase 2の合格

実データを使ったend-to-end demo、Windows build、証拠timeline、既知の制限を含むレビューpacketを渡し、ユーザーが`Phase 2 approved`と明示した時点で合格とする。
...
flowchart LR
    ...
    R1 -->|approved| P2["Phase 2: find, try, and execute"]
...
|---|---|---|
| Phase 1 | local inventory、fixture catalog、Capability Compiler、Resolver、hard gate、Context Pack preview、event replay、user approval | 三つのfixtureで、既存資産を見つけ、不適合候補を棄却し、根拠付きResolutionを出すところまで |
| Phase 2 | Phase 1に加え、承認済みlive source、Audit、sandbox Audition、Codex runtime evidence、Desktop | 実在候補を試し、本物のCodex turnと受入証拠をつなぐところまで |
...
対策は、Phase 1 rendererを再利用し、Electron shellはPhase 2の最後に載せること。独自UI frameworkを作らない。
...
- auditとsandbox Auditionを通して採用できる。
...
- Windowsアプリとしてsample projectを実行できる。
...
- Build WeekのdesktopはElectronを使う。
- V4新規stateはappend-only event journalと再構築可能なprojectionを使う。
- SQLite全面移行は実測後に判断する。
- 各phaseはユーザーの明示的承認がなければ進めない。
- Phase 1はlocal、Phase 2はexternalとexecution、Phase 3はlearningとintentに分ける。
