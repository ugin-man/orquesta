...
| taskの状態filter | 記録 > タスク上部 | すべて、完了、未完了、詳細state | task query | 各0件を表示 | 一つのfilterで状態を切り替えられる | 実装済み、再確認待ち |
| taskの二列表示 | 記録 > タスク本文 | task card | task query | 空表示 | desktop幅で二列を維持し、詳細popupで崩れない | 実装済み、再確認待ち |
