...
| --- | --- | --- | --- |
| タスク | 未完了、完了、全体のどれがどう進んだか | 三つの主切り替えと二列card grid | 一列だけの長いcard一覧 |
...
上部に、最もよく使う三つの切り替えを常設する。

1. 未完了
2. 完了
3. すべて

初期表示は未完了にする。完了はcanonical stateが`accepted`のtaskとする。failed、blocked、review待ちを完了へ混ぜない。未完了にはaccepted以外が入るが、失敗やblockedであることはcard上ではっきり区別する。

その下に、検索、agent、詳細state、更新期間、並び順を置く。三つの主切り替えと細かいstate filterを同じ階層に並べない。
...
cardを押すと右側から詳細drawerを開く。drawer表示中はgridを一列へ縮め、選択したtaskと周辺taskを同時に見られるようにする。利用可能な横幅が1100px未満の場合は最初から一列にする。
...
| task履歴 | 記録 > タスク | dock、Now、task chip | task query | 0件表示 | taskを検索できる | 部分実装 |
| taskの主切り替え | 記録 > タスク上部 | 未完了、完了、すべて | task query | 各0件を表示 | 三状態を一回で切り替えられる | 未実装 |
| taskの二列表示 | 記録 > タスク本文 | task card | task query | 空表示 | desktop幅で二列、狭い幅で一列になる | 未実装 |
| error履歴 | 記録 > エラー | dock、通知、関連task | failure query | 0件表示 | 反復回数を読める | 未実装 |
| errorの主切り替え | 記録 > エラー上部 | 未解決、反復、解決済み、すべて | failure query | 各0件を表示 | 状態を混同せず切り替えられる | 未実装 |
| 会話履歴 | 記録 > 会話 | dock、Composer、通知 | conversation query | 0件表示 | 実threadを読める | 部分実装 |
| 会話先切り替え | 記録 > 会話左側 | 会話先一覧 | conversation index | 会話先0件を表示 | 会話画面内で相手を変えられる | 未実装 |
| 記録の種類切り替え | 記録上部 | タスク、エラー、会話、ユーザー判断、タイムライン | record summaries | 各0件を表示 | 最上位画面を移動せず切り替えられる | 未実装 |
| 設定 | 設定 | dock | settings store | 初期値を表示 | 言語を変更できる | 未実装 |
