...
| error履歴 | 記録 > エラー | dock、通知、関連task | failure query | 0件表示 | 反復回数を読める | 実装済み、ユーザー確認待ち |
| errorの主切り替え | 記録 > エラー上部 | 未解決、反復、解決済み、すべて | failure query | 各0件を表示 | 状態を混同せず切り替えられる | 実装済み、ユーザー確認待ち |
