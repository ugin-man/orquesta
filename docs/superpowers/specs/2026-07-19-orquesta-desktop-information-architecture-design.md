...
Project Routeは古く、主要機能でもない。一機能だけの「その他」workspaceは作らず、`設定 > 起動とproject`から既存overlayを開く。設定内容をまだ決めていないsectionは、状態表示で無理に埋めない。現在値だけを見る情報は`状態と診断`へ集約する。
...
- Project Route一つのために「その他」を最上位画面として維持している
- 変更できない状態表示を設定項目のように並べ、設定を無理に成立させている
...
### 案2: Home、ユーザータスク、記録、設定に整理する

現在の外観とHomeを残し、Tasks、Failures、会話を「記録」に統合する。ユーザーが行うことは「ユーザータスク」、環境変更と状態確認は「設定」に置く。古い補助機能であるProject Routeは`設定 > 起動とproject`から開く。
...
| 表示と将来の通知、Codex、起動設定 | 設定 | 表示しない |
| repository、runtime、接続、診断の現在値 | 設定 > 状態と診断 | 表示しない |
| Project Route | 設定 > 起動とproject | 表示しない |
...
flowchart LR
    ...
    Home -->|会話履歴| Records
    Settings -->|起動とproject| Route["Project Route"]
...
3. 記録
4. 設定
日常的に使うHome、ユーザータスク、記録を左側の一群にする。設定の前には小さい区切りを入れ、日常操作と環境操作を視覚的に分ける。

ユーザータスクだけ、未処理総数をbadgeに出す。0件でもアイコンと名前は消さない。記録と設定には通常badgeを付けない。
...
connection状態を両方に出さない。pillを押す場合は`設定 > 状態と診断`へ移動する。Project Statusはproject切り替えや設定の入口にしない。
...
Project Routeはここへ追加しない。`設定 > 起動とproject`から開く。
...
Team ManagementはMap下部内側のボタンからだけ開く。設定には同じ入口を作らない。組織構成、管理係の要否、role group、最終配置は別のMap再設計まで変更しない。
...
エラーはtaskと同じ二列cardで表示する。検索、未解決・反復・解決済み・すべて、severity、並び順は一列のtoolbarにまとめる。四つの状態件数はtoolbarの上に短いsummaryとして残す。

cardにはerror class、短い説明、severity、発生回数、最後の発生、修復状態を表示する。cardを押すと中央に大きな詳細popupを開く。popup外側、右上の閉じるbutton、Escのどれでも閉じられるようにし、詳細表示によって一覧を一列へ組み替えない。
...
絞り込み結果を最初から全件DOMへ出さない。最初は200件まで表示し、ユーザー操作で200件ずつ追加する。headerには全件数と現在の表示件数を分けて出し、filter変更時は最初の200件へ戻す。10,000件級で取得とsort自体を軽くするcursor APIは、task、error、conversation、判断を横断するbridge contractと合わせて別工程で追加する。

## 設定

設定は独立したworkspaceにする。設定、状態確認、低頻度のproject補助機能を一つの最上位画面へまとめる。

左側にsection一覧、右側に内容を表示する。設定は項目同士を比較する画面ではないため、card gridにしない。実際に変更できる項目、まだ内容を設計していないsection、現在状態だけを示すread-only情報を見た目と置き場所で分ける。
...
- section名は残す
- 設定内容を決めるまでは短い未設計表示だけにし、現在動作を設定項目のように見せない

### Codex接続

- section名は残す
- 設定内容を決めるまでは短い未設計表示だけにし、runtime状態をここへ分散しない
...
- Project Routeを開く操作
- 将来の起動設定は、内容を決めるまでは短い未設計表示だけにする
...
実modelが確認できない場合は推測値を出さない。canonical write contractがない設定は作らず、未設計表示にする。read-onlyの状態は`状態と診断`へ集約し、設定項目と同じ見た目にしない。

## その他の廃止

Project Route一つのために最上位workspaceを維持するコストが見合わないため、「その他」は廃止する。Project Route本体とoverlayは変更せず、入口だけを`設定 > 起動とproject`へ移す。

Team ManagementはHomeのMap内だけに置く。Operationsと診断は`設定 > 状態と診断`へ置く。単に置き場所が決まらない機能を、新しい最上位workspaceとして追加してはいけない。
...
| 設定 > 起動とprojectのProject Route | Project Route overlay |
...
settings?section=startup&tool=project-route
```
...
| `WorkspaceSurface` | 同じ外枠を使い、ユーザータスク、記録、設定を表示 |

削除するのは、独立したTasks、Failures、会話workspaceと「その他」workspaceである。機能データは削除せず、新しい正式な置き場所へ移す。
...
- 言語を設定へ移動
- 変更できない状態を`状態と診断`へ集約
- 未設計の通知、Codex、起動設定を無理に現在状態で埋めない
- Operationsと診断を`状態と診断`へ移動
- Project Routeを`起動とproject`へ移動
- その他workspaceを廃止
...
- unit testは200件すべて通過
- production buildは通過
- browser操作は現行16シナリオすべて通過。1366 x 768、1440 x 900、全体scroll、keyboard、focus、project切り替えを含む
- visual比較は7画面すべて通過
- Electronは7シナリオすべて通過。起動、実repository読込、初回project選択、復旧画面、Codex送信と会話履歴、200%表示、送信中の応答性を含む
- I5では製品コードを変更していない。承認済み導線に古いtestを合わせ、承認済み画面のbaselineを更新した
- Map、pointer、memoryの長時間再測定は、今回Mapを変更しておらず症状も再発していないため実施していない
...
| --- | --- | --- | --- | --- | --- | --- |
| 質問件数 | Homeのユーザータスク概要 | 質問件数 | user task projection | 質問0を表示 | Homeだけで件数を読める | 実装済み、ユーザー承認済み |
| Team Management | Map下部内側 | Map内ボタン | agent projection | agent 0を表示 | 入口が一つだけ | 重複削除済み、ユーザー承認済み |
...
| ユーザー判断履歴 | 記録 > ユーザー判断 | 記録内の種類切り替え | attention history query | 0件表示 | 元の依頼と判断結果を種類別に探せる | 可視UIと0件状態は確認済み、実データ確認待ち |
| 集約タイムライン | 記録 > タイムライン | 記録内の種類切り替え | task、failure、conversation、attention history | 0件表示 | 種類、期間、agent、taskで横断して探せる | 実装済み、ユーザー承認済み |
| 記録の種類切り替え | 記録上部 | タスク、エラー、会話、ユーザー判断、タイムライン | record summaries | 各0件を表示 | 最上位画面を移動せず切り替えられる | 実装済み、ユーザー承認済み |
| 設定 | 設定 | dock | locale、project snapshot、runtime info | 未設計sectionを短く表示 | section移動、言語変更、状態確認、Operations表示 | 実装済み、ユーザー承認済み |
| Project Route | 設定 > 起動とproject | dock > 設定 | phase projection | 不足を明記 | 既存Routeを開ける | 入口移動済み、ユーザー承認済み |
...
- 設定が独立した画面として存在する
- 最上位ドックに「その他」がなく、Project Routeは設定の起動とprojectから開ける
