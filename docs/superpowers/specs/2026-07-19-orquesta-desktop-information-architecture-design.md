...
| 会話履歴 | 記録 > 会話 | dock、Composer、通知 | conversation query | 0件表示 | 実threadを読める | 可視UI実装済み、ユーザー確認待ち |
| 会話先切り替え | 記録 > 会話左側 | 会話先一覧 | agent projection、後続で履歴channel index | 会話先0件を表示 | 会話画面内で相手を変えられる | 可視UI実装済み、ユーザー確認待ち |
