...
// packages/contracts/src/foundation-agent-ids.js
var require_foundation_agent_ids = __commonJS({
  "packages/contracts/src/foundation-agent-ids.js"(exports2, module2) {
    "use strict";
    module2.exports = Object.freeze(["orchestrator", "orquesta-admin", "user-support"]);
  }
});
...
var require_validator = __commonJS({
  "packages/contracts/src/validator.js"(exports2, module2) {
    ...
    var FOUNDATION_AGENT_IDS = require_foundation_agent_ids();
    ...
    var defaultSchemasDir = process.env.ORQUESTA_CONTRACTS_SCHEMA_DIR ? path.resolve(process.env.ORQUESTA_CONTRACTS_SCHEMA_DIR) : path.resolve(__dirname, "../schemas");
    ...
    function assertSupportedSchema(schema, schemaPath = "$") {
      ...
      for (const [keyword, value] of Object.entries(schema)) {
        ...
        if (["minItems", "maxItems", "minLength", "maxLength"].includes(keyword) && (!Number.isInteger(value) || value < 0)) {
    ...
    function validateSchema(schema, value, valuePath = "$") {
      ...
      if (schema.maxItems !== void 0 && Array.isArray(value) && value.length > schema.maxItems) {
    ...
    function sortedUniqueRefs(values) {
      return Array.isArray(values) && values.every((value, index) => {
        ...
        return codeUnitCompare(previous.kind, value.kind) < 0 || previous.kind === value.kind && codeUnitCompare(previous.id, value.id) < 0;
      });
    }
    function agentRegistryV3Errors(value) {
      ...
      const provenanceKinds = {
        foundation: /* @__PURE__ */ new Set(["project_bootstrap"]),
        workflow: /* @__PURE__ */ new Set(["workflow_run"]),
        inspection: /* @__PURE__ */ new Set(["inspection"]),
        user: /* @__PURE__ */ new Set(["user"]),
        controller: /* @__PURE__ */ new Set(["task", "organization_decision"])
      };
      ...
      for (const agent of agents) {
        ...
        if (agent.origin !== null && agent.created_from_ref !== null && !provenanceKinds[agent.origin]?.has(agent.created_from_ref?.kind)) {
    ...
    function organizationStateV3Errors(value) {
      ...
      const activeAgentTeamKeys = /* @__PURE__ */ new Set();
      const activeTeamOrdinalKeys = /* @__PURE__ */ new Set();
      ...
      for (const line of lines.values()) {
        ...
        if (line.status === "active" && line.owner_ref?.kind === "participant" && participants.get(line.owner_ref.id)?.lifecycle_state !== "active") {
      ...
      for (const team of teams.values()) {
        ...
        if (team.coordination_mode === "supervised" && (team.lead_agent_id === null || leads.length !== 1 || leads[0].agent_id !== team.lead_agent_id)) {
        ...
        if (team.coordination_mode === "workflow_managed" && (leads.length > 1 || team.lead_agent_id !== null && (leads.length !== 1 || leads[0].agent_id !== team.lead_agent_id))) {
      ...
      for (const membership of membershipRecords) {
        ...
        if (isValidUtcTimestamp(membership.active_from) && membership.active_to !== null && isValidUtcTimestamp(membership.active_to) && membership.active_to < membership.active_from) {
        ...
        if (membership.active_to === null) {
          ...
          const agentTeamKey = `${membership.agent_id}\0${membership.team_id}`;
          ...
          const teamOrdinalKey = `${membership.team_id}\0${membership.ordinal}`;
      ...
      const relationshipSignatures = /* @__PURE__ */ new Set();
      for (const relationship of relationshipRecords) {
        const signature = `${relationship.type}\0${relationship.subject_ref?.kind}\0${relationship.subject_ref?.id}\0${relationship.object_ref?.kind}\0${relationship.object_ref?.id}`;
        ...
        for (const [field, ref] of [["subject_ref", relationship.subject_ref], ["object_ref", relationship.object_ref]]) {
          const exists = ref?.kind === "participant" ? participants.has(ref.id) : ref?.kind === "line" ? lines.has(ref.id) : ref?.kind === "team" ? teams.has(ref.id) : true;
          if (!exists) {
            ...
            const current = ref?.kind === "participant" ? participants.get(ref.id)?.lifecycle_state === "active" : ref?.kind === "line" ? lines.get(ref.id)?.status !== "retired" : ref?.kind === "team" ? teams.get(ref.id)?.lifecycle_state === "active" : true;
            if (!current) {
    ...
    function formationStateErrors(value) {
      ...
      const errors = [];
      const seen = /* @__PURE__ */ new Set();
      const sourceSignatures = /* @__PURE__ */ new Set();
      ...
      for (const formation of formations) {
        ...
        const sourceSignature = `${formation.formation_kind}\0${formation.source_ref?.kind}\0${formation.source_ref?.id}`;
        ...
        if (formation.formation_kind === "work_cell" && formation.source_ref?.kind !== "workflow_run" || formation.formation_kind === "inspection" && formation.source_ref?.kind !== "task") {
        ...
        if (formation.coordination_mode === "supervised" && (formation.lead_agent_id === null || !formation.member_agent_ids?.includes(formation.lead_agent_id))) {
        ...
        if (isValidUtcTimestamp(formation.created_at) && formation.retired_at !== null && isValidUtcTimestamp(formation.retired_at) && formation.retired_at < formation.created_at) {
    ...
    var SESSION_BINDING_LIFECYCLE = Object.freeze({
    ...
    function sessionBindingLifecycleErrors(session, basePath) {
      ...
      if (session.ownership_status !== lifecycle.ownership || session.handoff_status !== lifecycle.handoff || !lifecycle.bindings.includes(session.binding_status) || session.accepts_new_work !== lifecycle.accepts) {
      ...
      const hasAcceptedEvidence = typeof session.thread_id === "string" && typeof session.handoff_turn_id === "string" && isValidUtcTimestamp(session.accepted_at);
      ...
      if (!isValidUtcTimestamp(session.created_at) || !isValidUtcTimestamp(session.updated_at)) {
        ...
        if (lifecycle.acceptedEvidence && isValidUtcTimestamp(session.accepted_at) && (session.accepted_at < session.created_at || session.accepted_at > session.updated_at)) {
      ...
      if (lifecycle.ownershipWindow === "none" && (session.ownership_started_at !== null || session.ownership_ended_at !== null)) {
        errors.push(schemaError(basePath, "session_binding_ownership_window", "candidate sessions cannot claim an ownership interval"));
      }
      if (lifecycle.ownershipWindow === "open" && (!ownershipStarted || session.ownership_ended_at !== null)) {
      ...
      if (lifecycle.ownershipWindow === "closed" && (!ownershipStarted || !ownershipEnded)) {
      ...
      if (ownershipStarted && isValidUtcTimestamp(session.accepted_at) && session.ownership_started_at < session.accepted_at) {
      ...
      if (ownershipStarted && isValidUtcTimestamp(session.created_at) && session.ownership_started_at < session.created_at) {
      ...
      if (ownershipStarted && isValidUtcTimestamp(session.updated_at) && session.ownership_started_at > session.updated_at) {
    ...
    function sessionBindingStateV1Errors(value) {
      ...
      const generationsByAgent = /* @__PURE__ */ new Map();
      const threadOwners = /* @__PURE__ */ new Map();
      const ownersByAgent = /* @__PURE__ */ new Map();
      const activeAgents = /* @__PURE__ */ new Set();
      const liveCandidatesByPredecessor = /* @__PURE__ */ new Map();
      const retryChildrenByFailedAttempt = /* @__PURE__ */ new Map();
      const initialFailedAttemptsByLineage = /* @__PURE__ */ new Map();
      const attemptsByLineage = /* @__PURE__ */ new Map();
      const activeRuntimeAuthorities = /* @__PURE__ */ new Set();
      const initialRequestOwners = /* @__PURE__ */ new Map();
      const initialTaskOwners = /* @__PURE__ */ new Map();
      ...
      const ordered = [...value.sessions].sort((left, right) => {
        ...
        return codeUnitCompare(String(leftKey[0]), String(rightKey[0])) || Number(leftKey[1]) - Number(rightKey[1]) || codeUnitCompare(String(leftKey[2]), String(rightKey[2]));
      });
      ...
      for (const [index, session] of value.sessions.entries()) {
        ...
        if (session.rotation_state !== "failed") {
          const generations = generationsByAgent.get(session.agent_id) || /* @__PURE__ */ new Set();
        ...
        const isBoundNonterminal = accepted && session.binding_status === "bound" && !["superseded", "failed", "retired"].includes(session.rotation_state);
        ...
        if (["successor_warming", "successor_verified"].includes(session.rotation_state) && session.replaces_session_id !== null) {
        ...
        if (session.retry_of_session_id !== null) {
          ...
          const key = `${session.agent_id}\0${session.session_generation}\0${session.replaces_session_id}`;
      ...
      for (const [index, session] of value.sessions.entries()) {
        ...
        if (session.replaces_session_id !== null) {
          ...
          if (!predecessor) {
            ...
            if (predecessor.provisioning_request_id !== session.provisioning_request_id || predecessor.placement_intent_id !== session.placement_intent_id || predecessor.task_id !== session.task_id) {
            ...
            if (typeof predecessor.runtime_authority_id === "string" && typeof session.runtime_authority_id === "string" && predecessor.runtime_authority_id !== session.runtime_authority_id) {
            ...
            if (session.rotation_state === "failed" && ["superseded", "retired"].includes(predecessor.ownership_status)) {
              if (!isValidUtcTimestamp(session.updated_at) || !isValidUtcTimestamp(predecessor.ownership_ended_at) || session.updated_at >= predecessor.ownership_ended_at) {
              ...
              if (isValidUtcTimestamp(session.created_at) && isValidUtcTimestamp(predecessor.updated_at) && session.created_at <= predecessor.updated_at) {
              ...
              if (session.ownership_status !== "candidate" || !["successor_warming", "successor_verified", "failed"].includes(session.rotation_state) || predecessor.ownership_status !== "owner" || predecessor.rotation_state !== "checkpointed") {
        ...
        if (session.retry_of_session_id !== null) {
          ...
          } else if (failedAttempt.rotation_state !== "failed" || failedAttempt.agent_id !== session.agent_id || failedAttempt.session_generation !== session.session_generation || failedAttempt.replaces_session_id !== session.replaces_session_id || !isValidUtcTimestamp(failedAttempt.updated_at) || !isValidUtcTimestamp(session.created_at) || session.created_at <= failedAttempt.updated_at) {
        ...
        if (session.replaced_by_session_id !== null) {
          ...
          if (!successor) {
            ...
            if (session.ownership_status !== "superseded" || !["owner", "superseded", "retired"].includes(successor.ownership_status)) {
...
var require_v2 = __commonJS({
  "packages/context-compiler/src/v2.js"(exports2, module2) {
    ...
    function sourceType(reference) {
      if ([".orquesta/state/tasks.json", ".orquesta/state/placement-tasks.json"].includes(reference)) return "task_record";
      ...
      return {
        ...
        rules: [
          ...
          },
          {
            ...
            match: [".orquesta/state/*.json", ".orquesta/state/*.jsonl", ".orquesta/project/*.json", ".orquesta/user_tasks/*.json", ".orquesta/failures/*.json", ".orquesta/vision/*.json"],
            lifecycle: "current",
            authority: "canonical",
