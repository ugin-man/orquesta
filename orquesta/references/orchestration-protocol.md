...
2. State Read
   - Apply the authority classification in `SKILL.md` Start From the Smallest Reliable Context. This protocol does not redefine task-controller, selected-project, or authority-gap detection.
   - Read `references/project-bootstrap.md` only after that classification selects the exact Desktop/runtime project root and its Foundation route.
...
- `compact_receipt`: when `wait_threads` is not callable, `send_message_to_thread` is callable, and a specialist report is required, resolve the current orchestrator binding through the SessionBinding authority and put its verified `thread_id` and optional `host_id` into the specialist handoff. The specialist writes durable evidence first, then sends exactly one compact receipt to wake the orchestrator.
...
For a Codex-hosted placement or rotation, submit the intent through the package-owned SessionBinding lifecycle. The host-created task ID and accepted handoff receipt are evidence consumed by that lifecycle; Skill scripts must not maintain a parallel rotation registry or bind sessions by directly editing project state. The predecessor remains owner until the package-owned acceptance transition succeeds.
...
Use the `canonical_state_root` resolved during State Read. It must contain `.orquesta/state/tasks.json` before task planning or writes.
...
Enter this route only when the authority classification in `SKILL.md` selects it. Execute the Required Route in `references/project-bootstrap.md`; this protocol does not maintain a second bootstrap classifier or state machine. After the package-owned transaction is ready, return to the ordinary operating loop and use PlacementIntent only when durable delegation is actually needed.
...
6. Synchronization
   - Route durable agent lifecycle changes through OrganizationStore V3.
   - Route specialist task lifecycle changes through TaskPort.
   - Route thread, generation, handoff, and ownership changes through SessionBindingStore.
...
Create a new persistent specialist only when all are true:
...
- The orchestrator can express one bounded PlacementIntent with executable acceptance criteria.

Submit semantic intent to the V3 controller instead of selecting a legacy roster action. The controller decides whether to reuse an existing owner or create a new agent, compiles the Placement Task, updates organization state atomically, provisions the session, and records the accepted SessionBinding. The current `organization.json` policy controls line creation; do not hard-code an approval rule in the skill. This route does not bypass Codex safety or command approvals.

The only live durable authorities for this route are OrganizationStore V3, TaskPort, and SessionBindingStore. Repeating the same PlacementIntent is idempotent. A specialist is not operational until its executable task is assigned and an accepted, bound owner session is readable through SessionBindingStore. Do not write `organization-decisions.json`, `roles.json`, `setup/provisioning_batch.json`, or legacy session projections as part of live placement.
...
If a new incident is recorded after `user-support` has already written a "no failures" or readiness report, that report is stale. Wake or message `user-support` to re-read incidents and user actions before relying on readiness.

## Report Freshness

Contract-owned files under `.orquesta/state/`, `.orquesta/failures/`, `.orquesta/vision/`, and `.orquesta/user_tasks/` are the current source of truth for their respective routes. This statement does not authorize direct writes to controller-owned state.

Reports under `.orquesta/reports/` are snapshots written at a point in time. Do not assume report text is current after later state edits, port changes, new incidents, thread status changes, or answer curation. Readiness claims should re-check the relevant controller-owned state and, when needed, send a narrow re-sync prompt to stale Foundation threads.
