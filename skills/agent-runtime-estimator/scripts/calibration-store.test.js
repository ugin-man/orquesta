...
const {
  ...
  selectCalibration,
  profileKey
} = require("./calibration-store");

function tempRoot() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "agent-runtime-estimator-"));
}
...
test("lookup uses the most specific compatible profile and does not mix models", () => {
...
test("missing active time still calibrates elapsed time", () => {
  const result = summarize([
    { version: 1, profile_key: "profile", profile: { model: "model", reasoning: "medium", task_class: "research", execution_mode: "interactive", tool_profile: "web" }, critical_path_units: 5, actual_elapsed_minutes: 15, actual_agent_active_minutes: null }
  ]);
