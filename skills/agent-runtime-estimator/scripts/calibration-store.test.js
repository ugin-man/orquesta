...
test("records observations outside the skill package and compacts them", () => {
  ...
  try {
    ...
    const key = profileKey({ model: "gpt-5.6-sol", reasoning: "high", task_class: "coding", execution_mode: "codex", tool_profile: "local-tests" });
    assert.equal(result.profiles[key].sample_count, 2);
    assert.deepEqual(result.profiles[key].elapsed_minutes_per_critical_unit, { p50: 2, p80: 3 });
    assert.deepEqual(result.profiles[key].active_minutes_per_critical_unit, { p50: 1.5, p80: 2, sample_count: 2 });
    assert.equal(readObservations(root).length, 2);
    assert.equal(loadCalibration(root).profiles[key].sample_count, 2);
...
test("exact lookup does not mix known models", () => {
  ...
  assert.equal(selected.profile_key, "sol");
  assert.equal(selected.fallback_level, "exact");
  assert.equal(selected.elapsed_minutes_per_critical_unit.p50, 2);
});
...
test("falls back across tool profiles without crossing model or reasoning", () => {
  const calibration = summarize([
    ...
    { version: 1, profile_key: "c", profile: { model: "gpt-5.6-terra", reasoning: "high", task_class: "coding", execution_mode: "codex", tool_profile: "other" }, critical_path_units: 2, actual_elapsed_minutes: 40, actual_agent_active_minutes: null }
  ]);
  ...
  assert.equal(selected.fallback_level, "model+reasoning+task_class+execution_mode");
  assert.equal(selected.sample_count, 2);
  assert.equal(selected.elapsed_minutes_per_critical_unit.p50, 3);
});
...
test("missing active time still calibrates elapsed time", () => {
  ...
  assert.equal(result.profiles.profile.elapsed_minutes_per_critical_unit.p50, 3);
  assert.equal(result.profiles.profile.active_minutes_per_critical_unit, null);
});
