...
function appendObservation(observation, root = DEFAULT_ROOT) {
  const units = nonNegative(observation.critical_path_units, "critical_path_units");
  ...
  const record = {
    ...
    actual_elapsed_minutes: nonNegative(observation.actual_elapsed_minutes, "actual_elapsed_minutes"),
    actual_agent_active_minutes: observation.actual_agent_active_minutes == null ? null : nonNegative(observation.actual_agent_active_minutes, "actual_agent_active_minutes")
  };
  ensureRoot(root);
...
function readObservations(root = DEFAULT_ROOT) {
  try { return fs.readFileSync(path.join(root, "history.jsonl"), "utf8").split(/\r?\n/u).filter(Boolean).map(JSON.parse).filter((item) => item && item.version === VERSION && item.profile_key && item.critical_path_units > 0); }
  catch (error) { if (error && error.code === "ENOENT") return []; throw error; }
}

function summarize(observations) {
  const groups = new Map();
  for (const item of observations) { const items = groups.get(item.profile_key) || []; items.push(item); groups.set(item.profile_key, items); }
  const profiles = {};
  for (const [key, items] of groups) {
    const elapsed = items.map((item) => item.actual_elapsed_minutes / item.critical_path_units).sort((a, b) => a - b);
    const active = items.filter((item) => item.actual_agent_active_minutes != null).map((item) => item.actual_agent_active_minutes / item.critical_path_units).sort((a, b) => a - b);
    profiles[key] = {
      profile: items[0].profile,
      sample_count: elapsed.length,
      elapsed_minutes_per_critical_unit: { p50: percentile(elapsed, 0.5), p80: percentile(elapsed, 0.8) },
      active_minutes_per_critical_unit: active.length ? { p50: percentile(active, 0.5), p80: percentile(active, 0.8), sample_count: active.length } : null
    };
  }
...
function compact(root = DEFAULT_ROOT) {
  ensureRoot(root); const result = summarize(readObservations(root)); const target = path.join(root, "calibration.json"); const temp = `${target}.tmp`;
...
function selectCalibration(requestedInput, calibration = loadCalibration()) {
  const requested = profile(requestedInput);
  const entries = Object.entries(calibration.profiles || {});
  const levels = [FIELDS, ["model", "reasoning", "task_class", "execution_mode"], ["model", "reasoning", "task_class"], ["model", "task_class"], ["task_class"]];
  for (const fields of levels) {
    const matches = entries.filter(([, entry]) => fields.every((field) => requested[field] === "unknown" || part(entry.profile?.[field]) === requested[field]));
    if (!matches.length) continue;
    if (fields.length === FIELDS.length && matches.length === 1) return { profile_key: matches[0][0], fallback_level: "exact", ...matches[0][1] };
    const total = matches.reduce((sum, [, entry]) => sum + Number(entry.sample_count || 0), 0);
    if (!total) continue;
    ...
    return {
      profile_key: `${fields.map((field) => requested[field]).join("|")}|*`, fallback_level: fields.join("+"), sample_count: total,
      elapsed_minutes_per_critical_unit: { p50: weighted("elapsed_minutes_per_critical_unit", "p50"), p80: weighted("elapsed_minutes_per_critical_unit", "p80") },
      active_minutes_per_critical_unit: null
    };
  }
  return null;
}
...
function main(argv) {
  const command = argv[2]; const f = flags(argv.slice(3)); const root = f.home ? path.resolve(String(f.home)) : DEFAULT_ROOT;
  if (command === "record") { process.stdout.write(`${JSON.stringify(appendObservation({ model: f.model, reasoning: f.reasoning, task_class: f.task_class, execution_mode: f.execution_mode, tool_profile: f.tool_profile, critical_path_units: f.units, actual_elapsed_minutes: f.elapsed, actual_agent_active_minutes: f.active }, root))}\n`); return; }
  if (command === "compact") { process.stdout.write(`${JSON.stringify({ profiles: Object.keys(compact(root).profiles).length })}\n`); return; }
  if (command === "lookup") { process.stdout.write(`${JSON.stringify(selectCalibration({ model: f.model, reasoning: f.reasoning, task_class: f.task_class, execution_mode: f.execution_mode, tool_profile: f.tool_profile }, loadCalibration(root)))}\n`); return; }
