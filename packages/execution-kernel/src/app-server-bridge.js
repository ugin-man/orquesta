...
function dispatchPayloadConflict(dispatchId) {
  const error = new Error(`dispatch_payload_conflict:${dispatchId}`);
  error.code = "dispatch_payload_conflict";
  return error;
}
...
function adapterFailure(result, operation) {
  ...
  error.evidence = clone(result?.evidence ?? {});
  return error;
}

function requireResolvedStart(session, taskId) {
  const failure = session?.last_start_failure;
  if (failure?.code === "runtime_outcome_unknown"
    || (session?.start_turn_stage === "issued" && failure?.dispatch_accepted !== false)) {
    const error = new Error(`turn_start_recovery_required:${taskId}`);
    error.code = "runtime_outcome_unknown";
    error.operation = "startTurn";
    error.evidence = { dispatch_accepted: failure?.dispatch_accepted ?? null };
    throw error;
  }
}
...
function createAppServerExecutionBridge({
  ...
  async function performStart(dispatch, taskId, dispatchId, attempt, inputSignature) {
    ...
    requireResolvedStart(session, taskId);
    ...
    if (!session) {
      ...
      session = {
        ...
        accepted_turns: 0,
        start_turn_stage: "not_issued",
        last_start_failure: null,
    ...
    dispatchInputs.set(dispatchId, inputSignature);
    dispatches.set(dispatchId, record);
    byCorrelation.set(correlationId, record);

    let started;
    session.start_turn_stage = "issued";
    session.last_start_failure = null;
    try {
      ...
      session.last_start_failure = {
        code: error?.code || "codex_adapter_failed",
        dispatch_accepted: typeof error?.evidence?.dispatch_accepted === "boolean"
          ? error.evidence.dispatch_accepted
          : null,
        operation: error?.operation || "startTurn",
      };
      dispatches.delete(dispatchId);
      dispatchInputs.delete(dispatchId);
      byCorrelation.delete(correlationId);
      throw error;
    }
    record.turn_id = nonempty(started.turn_id, "startTurn.turn_id");
    session.accepted_turns += 1;
    session.start_turn_stage = "accepted";
    session.last_start_failure = null;
  ...
  async function start(dispatch) {
    ...
    const immutableDispatch = clone(dispatch);
    const inputSignature = canonicalJson(immutableDispatch);
    const pending = dispatchReservations.get(dispatchId);
    if (pending) {
      if (pending.input_signature !== inputSignature) {
        throw dispatchPayloadConflict(dispatchId);
      }
      return pending.promise;
    }
    if (dispatches.has(dispatchId)) {
      if (dispatchInputs.get(dispatchId) !== inputSignature) {
        throw dispatchPayloadConflict(dispatchId);
      }
      throw new Error(`duplicate_dispatch:${dispatchId}`);
    }
    if ([...dispatchReservations.values()].some((reservation) => reservation.task_id === taskId)) {
      const error = new Error(`task_start_in_progress:${taskId}`);
      error.code = "task_start_in_progress";
      throw error;
    }
    ...
    // Defer all adapter and task-resolution work until after the reservation is
    // visible. Concurrent callers can now join this exact delivery, while a
    // different dispatch for the same task cannot create a competing session.
    const reservation = {
      task_id: taskId,
      input_signature: inputSignature,
      promise: null,
    };
    reservation.promise = Promise.resolve().then(() => performStart(
      immutableDispatch,
      taskId,
      dispatchId,
      attempt,
    ...
    dispatchReservations.set(dispatchId, reservation);

    try {
      return await reservation.promise;
    } finally {
      if (dispatchReservations.get(dispatchId) === reservation) {
        dispatchReservations.delete(dispatchId);
      }
    }
  }
