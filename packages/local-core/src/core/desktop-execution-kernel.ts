...
import type {
  ...
  RuntimeThreadNotification as RuntimeNotification
} from './protocol';
import type { ProjectCodexThread } from './session-binding-resolver';
import {
  PROJECT_STORAGE,
  projectStoragePath
...
export class DesktopPreDispatchError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'DesktopPreDispatchError';
  }
}
...
function statePath(rootPath: string): string {
  return projectStoragePath(rootPath, PROJECT_STORAGE.executionKernel);
}
...
export class DesktopExecutionKernelController {
  ...
  dispatch(input: DispatchInput): Promise<RuntimeDispatchResult> {
    ...
    return this.enqueue(async () => {
      ...
      const activeCount = Object.values(state.tasks).filter((task) => ACTIVE_STATES.has(task.state)).length;
      if (!state.tasks[taskId] && activeCount >= state.max_concurrent) {
        throw new DesktopPreDispatchError('Desktop execution capacity is full; the message was not queued or sent');
      }
  ...
  observe(notification: RuntimeNotification, allowedRootPath?: string): Promise<void> {
    if (!this.enabled) return Promise.resolve();
    return this.enqueue(async () => {
      const match = this.taskForRuntimeIdentity(notification.threadId, notification.turnId);
      if (!match) return;
      if (allowedRootPath && path.resolve(match.rootPath) !== path.resolve(allowedRootPath)) return;
