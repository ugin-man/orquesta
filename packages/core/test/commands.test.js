...
test("Phase Review user decisions bind their current journal revision and record their exact transition event", () => {
  ...
  const pack = { context_pack_id: "CP-current-revision", task_intent_id: "TI-current-revision", owner_agent_id: "implementation-001", objective: "Current revision", acceptance_criteria: ["Verify"], adopted_decisions: [], capability_resolutions: [], required_reading: [], relevant_state_excerpts: [], interfaces: [], allowed_files: [], forbidden_actions: [], excluded_context: [], evidence_requirements: ["Verify"], provenance: [], token_budget: null, expires_at: null, status: "draft" };
  boundary.execute({ command_id: "current-pack", name: "context-pack.preview", payload: { context_pack: pack } });
  const packet = { artifact_ref: "artifact:current", artifact_hash: "8".repeat(64), kind: "phase_review_packet" };
  store.commit({ expected_revision: 1, batch_id: "current-packet", actor: { type: "agent", id: "implementation" }, correlation_id: "current-packet", events: [{ event_id: "current-packet-event", schema_version: 1, type: "artifact.produced", payload: { artifact: packet, responsibility: "implementation" }, evidence_refs: [packet.artifact_ref] }] });
  ...
  store.commit({ expected_revision: 4, batch_id: "current-intervening", actor: { type: "agent", id: "implementation" }, correlation_id: "current-intervening", events: [{ event_id: "current-intervening-event", schema_version: 1, type: "artifact.produced", payload: { artifact: { artifact_ref: "artifact:other", artifact_hash: "9".repeat(64), kind: "phase_review_packet" }, responsibility: "implementation" }, evidence_refs: ["artifact:other"] }] });
  ...
  assert.equal(observedTarget.target_revision, 5);
  ...
  const state = boundary.replay();
  assert.equal(state.phase_reviews[0].review_cycle_revision, 5);
...
test("all state-changing approval and Phase Review retries are idempotent before verifier or active-cycle checks", () => {
  ...
  const pack = { context_pack_id: "CP-retry", task_intent_id: "TI-retry", owner_agent_id: "implementation-001", objective: "Retry", acceptance_criteria: ["Verify"], adopted_decisions: [], capability_resolutions: [], required_reading: [], relevant_state_excerpts: [], interfaces: [], allowed_files: [], forbidden_actions: [], excluded_context: [], evidence_requirements: ["Verify"], provenance: [], token_budget: null, expires_at: null, status: "draft" };
  boundary.execute({ command_id: "retry-pack", name: "context-pack.preview", payload: { context_pack: pack } });
