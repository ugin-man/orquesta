...
function draftContextPack() {
  return {
    context_pack_id: "CP-draft", task_intent_id: "TI-draft", owner_agent_id: "implementation-001", objective: "Draft objective",
    acceptance_criteria: ["Verify draft"], adopted_decisions: [], capability_resolutions: [], required_reading: [],
    relevant_state_excerpts: [], interfaces: [], allowed_files: [], forbidden_actions: [], excluded_context: [],
    evidence_requirements: ["Verify draft"], provenance: [], token_budget: null, expires_at: null, status: "draft",
  };
}
...
test("vertical lifecycle commits through the real EventStore and replay matches after projections are removed", () => {
  ...
  assert.throws(() => boundary.execute({ command_id: "invalid-pack", name: "context-pack.preview", payload: { context_pack: { context_pack_id: "CP", status: "draft" } } }), { code: "CORE_CONTEXT_PACK_INVALID" });
  const contextPack = draftContextPack();
  boundary.execute({ command_id: "5", name: "context-pack.preview", payload: { context_pack: contextPack } });
...
test("an active Phase Review cannot be replaced by a second request for the same phase", () => {
  ...
  const pack = draftContextPack();
  boundary.execute({ command_id: "pack-active", name: "context-pack.preview", payload: { context_pack: pack } });
  const artifact = { artifact_ref: "artifact:active", artifact_hash: "e".repeat(64), kind: "phase_review_packet" };
  store.commit({ expected_revision: boundary.replay().timeline.at(-1).sequence, batch_id: "active-packet-seed", actor: { type: "agent", id: "packet" }, correlation_id: "active-packet-seed", events: [{ event_id: "active-packet-event", schema_version: 1, type: "artifact.produced", payload: { artifact, responsibility: "implementation" }, evidence_refs: [artifact.artifact_ref] }] });
