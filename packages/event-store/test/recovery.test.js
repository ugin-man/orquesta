...
test("dead lock recovery treats its commit outcome as unknown but rejects malformed identity", () => {
  const variants = [
    ["nonce", { owner_pid: 999999, lock_version: 2, owner_incarnation: "b".repeat(64), nonce: "not-hex" }],
  ...
  try {
    writeLock(lagging, { owner_pid: 999999, target_revision: 99 });
    const store = createEventStore({ stateRoot: lagging.root, hostId: "recovery-host" });
    const inspection = inspectReadOnly(store, lagging.root, "stale_lock");
    assert.equal(inspection.summary.lock.eligible_for_explicit_release, true);
  } finally { lagging.cleanup(); }
});
...
test("backup restore requires a valid matching backup and preserves all evidence when copy confirmation fails", () => {
  const f = fixture();
  try {
    ...
    assert.throws(() => store.applyRecovery(recoveryInput(inspection)), { code: "EPERM" });
...
test("backup restore resumes safely after a stop between quarantine copy and atomic replacement", () => {
  const f = fixture();
  try {
    ...
    const stopping = createEventStore({ stateRoot: f.root, testRecoveryHooks: { afterQuarantineMove() { throw Object.assign(new Error("stop after copy"), { code: "EVENT_TEST_STOP_AFTER_MOVE" }); } } }); const first = inspectReadOnly(stopping, f.root, "restore_backup_retry"); assert.throws(() => stopping.applyRecovery(recoveryInput(first)), { code: "EVENT_TEST_STOP_AFTER_MOVE" }); assert.equal(fs.existsSync(f.journal), true);
...
test("an unrelated quarantine copy cannot replace the exact backup and pending lineage", () => {
  const f = fixture();
  try {
    ...
    const resumed = createEventStore({ stateRoot: f.root });
    const second = inspectReadOnly(resumed, f.root, "restore_backup_retry");
    assert.equal(resumed.applyRecovery(recoveryInput(second)).status, "committed");
  } finally { f.cleanup(); }
});
...
test("conflict completion keeps original artifacts beside verified quarantine evidence", () => {
  const f = fixture();
  try {
    ...
    assert.equal(blocked.diagnostics.conflict_reason, null);
    assert.equal(fs.existsSync(f.journal), true); assert.equal(fs.existsSync(pendingPath), true);
    assert.equal(fs.readdirSync(f.root).some((name) => name.includes(".conflict-")), false);
  } finally { f.cleanup(); }
});
