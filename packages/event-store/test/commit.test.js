...
test("optional projection preflight rejects invalid batches before durable evidence is written", () => {
  const f = fixture();
  try {
    const store = createEventStore({
      ...
      reducers: {
        "task.updated"(state, event) {
          if (event.payload.reject === true) {
            throw Object.assign(new Error("candidate projection rejected"), {
              code: "CANDIDATE_PROJECTION_REJECTED",
            });
          }
    ...
    const rejected = batch({
      batch_id: "preflight-rejected",
      event_id: "preflight-rejected-event",
    });
    rejected.events[0].payload = { reject: true };
    assert.throws(() => store.commit(rejected), { code: "CANDIDATE_PROJECTION_REJECTED" });
    assert.equal(fs.existsSync(f.journalPath), false);
    assert.deepEqual(pendingFiles(f.root), []);
    assert.equal(store.replay().watermark.journal_sequence, 0);

    const accepted = batch({
      batch_id: "preflight-accepted",
      event_id: "preflight-accepted-event",
    });
    assert.deepEqual(store.commit(accepted), { status: "committed", sequence: 1 });
    assert.equal(store.replay().state.total, 1);
  } finally {
    f.cleanup();
  }
});

test("projection preflight configuration is explicit", () => {
  const f = fixture();
  try {
    assert.throws(
      () => createEventStore({ stateRoot: f.root, preflightProjection: "yes" }),
      /preflightProjection must be a boolean/u,
    );
  } finally {
    f.cleanup();
  }
...
test("records lock ownership metadata, refuses stale locks, and preserves a replacement nonce on release", () => {
  ...
  try {
    ...
    fs.writeFileSync(`${f.journalPath}.lock`, `${JSON.stringify({ ...lock.metadata, nonce: "b".repeat(32) })}\n`, "utf8");
    assert.throws(() => releaseJournalLock(lock), { code: "EVENT_LOCK_OWNERSHIP_LOST" });
    assert.equal(inspectJournalLock(f.journalPath).metadata.nonce, "b".repeat(32));
    fs.unlinkSync(`${f.journalPath}.lock`);
...
test("does not move a replacement owner during release transition", () => {
  ...
  try {
    const lock = acquireJournalLock({ journalPath: f.journalPath, hostId: "host-a", targetRevision: 0, timeoutMs: 20 });
    const replacement = { ...lock.metadata, nonce: "c".repeat(32) };
...
test("preserves a replacement owner swapped after the final release recheck", () => {
  const f = fixture();
  try {
    const lock = acquireJournalLock({ journalPath: f.journalPath, hostId: "host-a", targetRevision: 0, timeoutMs: 20 });
    const replacement = { ...lock.metadata, nonce: "d".repeat(32) };
    assert.throws(() => releaseJournalLock(lock, {
      testHooks: {
        beforeReleaseMove() {
          fs.unlinkSync(lock.lockPath);
...
test("a writer that loses its lock cannot overwrite a later successful commit", () => {
  const f = fixture();
  try {
    const winnerRequest = batch({ batch_id: "replacement-winner", event_id: "replacement-winner-event" });
    const contender = createEventStore({ stateRoot: f.root, workspaceId: "workspace-a", lockTimeoutMs: 20 });
    const staleWriter = createEventStore({
      stateRoot: f.root,
      workspaceId: "workspace-a",
      testLockHooks: {
        afterRecoveryEvidenceCheck({ journalPath }) {
          fs.unlinkSync(`${journalPath}.lock`);
          for (const name of fs.readdirSync(f.root)) {
            if (name.includes(".transition-")) fs.unlinkSync(path.join(f.root, name));
          }
          assert.deepEqual(contender.commit(winnerRequest), { status: "committed", sequence: 1 });
        },
      },
    });
    assert.throws(
      () => staleWriter.commit(batch({ batch_id: "stale-loser", event_id: "stale-loser-event" })),
      { code: "EVENT_LOCK_OWNERSHIP_LOST" },
    );
    const lines = readLines(f.journalPath);
    assert.equal(lines.length, 1);
    assert.equal(lines[0].batch_id, winnerRequest.batch_id);
  } finally {
    f.cleanup();
  }
});
