...
function consumeRecoveryPending(pendingPath, pendingBytes, lock) {
  ...
  try { fs.unlinkSync(pendingPath); } catch (error) { throw eventStoreError("EVENT_PENDING_CONSUME_FAILED", "Could not delete verified recovery pending evidence", { pending_path: pendingPath, cause_code: error.code || null }); }
  return { pendingPath };
}
...
function physicalJournalText(journalPath) {
  try {
    return new TextDecoder("utf-8", { fatal: true }).decode(fs.readFileSync(journalPath));
  } catch (error) {
...
function assertJournalAuthority(lock, journalPath, expectedText) {
  assertJournalLockOwned(lock);
  const actualText = physicalJournalText(journalPath);
  if (actualText !== expectedText) {
    throw eventStoreError("EVENT_JOURNAL_SOURCE_CHANGED", "Journal bytes changed while the current writer held publication authority", {
      journal_path: journalPath,
      expected_sha256: sha256Text(expectedText),
      actual_sha256: sha256Text(actualText),
    });
  }
}

function removeControlledArtifacts({ pendingPath, journalPath, priorJournalText, expectedJournalText, lock }) {
  assertJournalAuthority(lock, journalPath, expectedJournalText);
  ...
  if (priorJournalText === "") unlinkIfPresent(journalPath);
    ...
    onBeforeRename() { assertJournalAuthority(lock, journalPath, expectedJournalText); },
  });
  assertJournalAuthority(lock, journalPath, priorJournalText);
}
...
function createEventStore(options = {}) {
  ...
  if (options.preflightProjection !== undefined
      && typeof options.preflightProjection !== "boolean") {
    throw new TypeError("preflightProjection must be a boolean");
  }
  ...
  const lockTimeoutMs = options.lockTimeoutMs === undefined ? DEFAULT_LOCK_TIMEOUT_MS : options.lockTimeoutMs;
  ...
  const replay = ({
    ...
    claimedEventPrefixes = [],
    afterCursor = null,
  } = {}) => projectionStore.replay(
    readJournal,
    journalPath,
    reducers,
    initialState,
    { claimedEventPrefixes, afterCursor },
  );
  const rebuildProjectionsLocked = (
    lock,
    { reducers = defaultReducers, initialState = defaultInitialState } = {},
    expectedJournalText,
  ) => {
    const assertAuthority = () => assertJournalAuthority(lock, journalPath, expectedJournalText);
    assertAuthority();
    return projectionStore.rebuild(
      readJournal,
      stateRoot,
      journalPath,
      reducers,
      initialState,
      { onBeforePublish: assertAuthority, onAfterPublish: assertAuthority },
    );
  };
  const rebuildProjections = ({
    ...
    let primaryError;
    try {
      const preliminary = readJournal(journalPath);
      lock = acquireJournalLock({
        journalPath,
        hostId,
        targetRevision: preliminary.entries.length,
        timeoutMs: lockTimeoutMs,
        testHooks: options.testLockHooks,
      });
      if (typeof beforeLockedRebuild === "function") beforeLockedRebuild(lock);
      const journal = readJournal(journalPath);
      guard = armJournalLockGuard(lock, { testHooks: options.testLockHooks });
      rebuilt = rebuildProjectionsLocked(lock, { reducers, initialState }, journal.text);
    } catch (error) {
      primaryError = error;
    }
    if (guard) {
      try { disarmJournalLockGuard(guard, { testHooks: options.testLockHooks }); }
      catch (error) { if (!primaryError) primaryError = error; }
    }
    if (lock) {
      try { releaseJournalLock(lock, { testHooks: options.testLockHooks }); }
      catch (error) {
  ...
  const preflightProjection = (entry) => {
    if (options.preflightProjection !== true) return;
    let state = replay().state;
    for (const event of entry.events) {
      const reducer = defaultReducers[event.type];
      if (typeof reducer === "function") state = reducer(state, event, entry);
    }
  };
  ...
  const api = {
    ...
    commit(request, internal = {}) {
      ...
      try {
        ...
        expectedJournalText = journal.text;
        publicationGuard = armJournalLockGuard(lock, { testHooks: options.testLockHooks });
        ...
        if (existing) {
          ...
          expectedJournalText = nextJournalText;
          assertJournalAuthority(lock, journalPath, expectedJournalText);
          ...
          rebuildProjectionsLocked(lock, {}, expectedJournalText);
          project(entry);
          assertJournalAuthority(lock, journalPath, expectedJournalText);
        ...
        if (recoveryPending && result?.status === "idempotent") rebuildProjectionsLocked(lock, {}, expectedJournalText);
        ...
        if (controlledFailure.error === error && pendingPath) {
          try {
            ...
            expectedJournalText = priorJournalText;
          } catch (cleanupError) {
      ...
      if (publicationGuard) {
        try {
          disarmJournalLockGuard(publicationGuard, { testHooks: options.testLockHooks });
        } catch (error) {
          if (primaryError) {
            attachSecondaryEvidence(primaryError, "guard_failure", {
