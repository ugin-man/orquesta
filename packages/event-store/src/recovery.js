...
function parseLock(lockPath, hostId, ignoreOwnedLock = null) {
  const journalPath = lockPath.endsWith(".lock") ? lockPath.slice(0, -5) : lockPath;
  const observed = inspectJournalLock(journalPath);
  if (ignoreOwnedLock
      && observed.transitions.length === 0
      && observed.metadata
      && ignoreOwnedLock.lockPath === lockPath
      && canonicalJson(observed.metadata) === canonicalJson(ignoreOwnedLock.metadata)) {
    return { exists: false, eligible: false, transitions: [] };
  }
  if (observed.transitions.length > 0) {
    const owner = classifyOwner(observed.metadata, hostId);
    let ownedPublicationTransitions = false;
    if (observed.metadata?.lock_version === 2 && ["dead", "incarnation_mismatch"].includes(owner)) {
      try {
        ownedPublicationTransitions = observed.transitions.every((transitionPath) => {
          if (!path.basename(transitionPath).startsWith(`${path.basename(lockPath)}.transition-`)) return false;
          const transition = readTransitionMetadata(transitionPath);
          return transition.owner_nonce === observed.metadata.nonce
            && transition.owner_pid === observed.metadata.owner_pid
            && transition.host_id === observed.metadata.host_id
            && transition.owner_incarnation === observed.metadata.owner_incarnation;
        });
      } catch {
        ownedPublicationTransitions = false;
      }
    }
    if (ownedPublicationTransitions) {
      return {
        exists: true,
        eligible: true,
        reason: owner === "incarnation_mismatch" ? "owner_incarnation_mismatch" : "dead_local_owner",
        metadata: observed.metadata,
        transitions: observed.transitions,
        commit_outcome: "unknown",
      };
    }
    return { exists: true, eligible: false, reason: "transition_present", metadata: observed.metadata || null, transitions: observed.transitions };
  }
  if (!observed.exists) return { exists: false, eligible: false, transitions: [] };
  if (!observed.metadata) return { exists: true, eligible: false, reason: "invalid_metadata", transitions: [] };
  const owner = classifyOwner(observed.metadata, hostId);
  if (owner === "remote") return { exists: true, eligible: false, reason: "host_unproven", metadata: observed.metadata, transitions: [] };
  if (owner === "live") return { exists: true, eligible: false, reason: "owner_live", metadata: observed.metadata, transitions: [] };
  if (owner === "unproven") return { exists: true, eligible: false, reason: "owner_unproven", metadata: observed.metadata, transitions: [] };
  return {
    exists: true,
    eligible: true,
    reason: owner === "incarnation_mismatch" ? "owner_incarnation_mismatch" : "dead_local_owner",
...
function finalizeLockEligibility(lock, currentRevision, pending) {
  ...
  return { ...lock, observed_revision: currentRevision, pending_kind: pending?.kind || null, commit_outcome: "unknown" };
}
...
function conflictProgress(manifest) {
  ...
  if (value.status === "user_decision") {
    if (states.some((state) => state.transition !== "absent")) return "completed_artifacts_present";
    return "complete";
  }
  if (states.some((state) => state.canonical === "replacement" || state.transition === "replacement")) return "replacement";
  return states.every((state) => (state.canonical === "exact" && state.transition === "absent")
    || (state.canonical === "absent" && ["absent", "exact"].includes(state.transition))) ? "ready" : "incomplete";
}
...
function transitionConflictArtifact(item, hooks, beforeHookName, afterHookName) {
  ...
  if (fs.existsSync(item.canonical_path) && !bytesEqualAt(item.canonical_path, expected)) {
    throw eventStoreError("RECOVERY_CONFLICT_VERIFY_FAILED", "Canonical conflict artifact changed while confirming quarantine evidence", { path: item.canonical_path });
  }
  if (fs.existsSync(item.transition_path) && !bytesEqualAt(item.transition_path, expected)) {
    throw eventStoreError("RECOVERY_CONFLICT_VERIFY_FAILED", "Historical conflict transition changed while confirming quarantine evidence", { transition_path: item.transition_path });
  }
}
...
function inspect({ stateRoot, journalPath, readJournal, replay, hostId, workspaceId, ignoreOwnedLock = null }) {
  const projection = projectionPath(stateRoot);
  const lockPath = `${journalPath}.lock`;
  const observedLock = ignoreOwnedLock ? inspectJournalLock(journalPath) : null;
  const canIgnoreLock = Boolean(ignoreOwnedLock
    && observedLock?.transitions.length === 0
    && observedLock.metadata
    && ignoreOwnedLock.lockPath === lockPath
    && canonicalJson(observedLock.metadata) === canonicalJson(ignoreOwnedLock.metadata));
  const artifacts = snapshot(stateRoot, journalPath, projection)
    .filter((artifact) => !(canIgnoreLock && artifact.path === lockPath));
  const id = recoveryId(artifacts);
  const diagnosis = diagnoseJournal(journalPath, readJournal);
  let lock = parseLock(lockPath, hostId, ignoreOwnedLock);
  const conflictPaths = fs.existsSync(stateRoot) ? fs.readdirSync(stateRoot).filter((name) => /conflicted copy/i.test(name)).sort().map((name) => path.join(stateRoot, name)) : [];
...
function apply({ inspection, input, stateRoot, journalPath, readJournal, replay, rebuild, retry, hostId, workspaceId, hooks }) {
  ...
  if (input.action === "quarantine_conflict") {
    ...
    if (progress === "incomplete" || progress === "ready") {
  ...
  if (input.action === "stale_lock") {
    ...
    const lockHandle = { journalPath, lockPath: `${journalPath}.lock`, metadata: lock.metadata };
    if (lock.transitions.length > 0) removeDeadJournalLockTransitions(lockHandle, lock.transitions);
    releaseJournalLock(lockHandle, { testHooks: hooks });
    const after = inspect({ stateRoot, journalPath, readJournal, replay, hostId, workspaceId });
    return { status: "lock_released", next_action: after.action, summary: after.summary };
  }
  if (input.action === "rebuild_projection") {
    const rebuilt = rebuild({
      beforeLockedRebuild(lock) {
        const lockedCurrent = inspect({ stateRoot, journalPath, readJournal, replay, hostId, workspaceId, ignoreOwnedLock: lock });
        if (lockedCurrent.recovery_id !== inspection.recovery_id) {
          throw eventStoreError("RECOVERY_STATE_CHANGED", "Recovery artifacts changed while acquiring rebuild authority");
        }
        if (lockedCurrent.action !== input.action) {
          throw eventStoreError("RECOVERY_ACTION_MISMATCH", "Recovery action changed while acquiring rebuild authority", {
            expected_action: input.action,
            actual_action: lockedCurrent.action,
          });
        }
      },
    });
    return result("projection_rebuilt", { rebuilt });
  }
  ...
  if (input.action === "restore_backup_retry") {
    ...
    if (fs.existsSync(journalPath)) {
      ...
      if (hooks?.beforeQuarantineMove) hooks.beforeQuarantineMove({ journalPath, copyPath });
      if (!fs.existsSync(journalPath) || !fs.readFileSync(journalPath).equals(broken)) throw eventStoreError("RECOVERY_STATE_CHANGED", "Broken journal changed after its quarantine copy was verified", { journal_path: journalPath });
