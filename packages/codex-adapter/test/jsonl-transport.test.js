...
test("observes both directions with exact request causality", async () => {
  const process = new FakeAppServerProcess();
  const frames = [];
  const transport = createJsonlTransport({
    process,
  ...
  await nextTurn();
  transport.respond(17, { decision: "decline" });

  assert.equal(serverRequests.length, 1);
  assert.deepEqual(frames.map((entry) => entry.direction), [
    "client_to_provider",
    "provider_to_client",
    "provider_to_client",
    "client_to_provider"
  ]);
  assert.deepEqual(frames[1].request, request);
  assert.deepEqual(frames[3].request, approval);
});

test("makes an in-flight dynamic request ID duplicate fail closed with one final response", async () => {
  const process = new FakeAppServerProcess();
  const transport = createJsonlTransport({ process });
  const requests = [];
  transport.onServerRequest((message) => requests.push(message));
  const dynamic = {
    id: "dynamic-duplicate",
    method: "item/tool/call",
    params: {
      threadId: "thread-1", turnId: "turn-1", tool: "orquesta_attachment_read",
      arguments: { capability: "a".repeat(64), cursor: null }, callId: "call-1"
    }
  };
  process.send(dynamic);
  process.send({ ...dynamic, params: { ...dynamic.params, arguments: { capability: "b".repeat(64), cursor: null } } });
  await nextTurn();
  assert.equal(requests.length, 1);
  const outcome = transport.respond(dynamic.id, {
    success: true,
    contentItems: [{ type: "inputText", text: "must-not-leak" }]
  });
  assert.deepEqual(outcome, { overridden: true });
  const response = process.clientMessages.find((message) => message.id === dynamic.id);
  ...
  await nextTurn();
  assert.equal(requests.length, 1);
  assert.equal(process.clientMessages.filter((message) => message.id === dynamic.id).length, 1);
});
...
test("shutdown kills a child which does not exit before the supplied timeout", async () => {
  ...
  await assert.rejects(transport.shutdown({ timeoutMs: 5 }), /drain could not be confirmed/i);
  assert.equal(process.endObserved, true);
...
test("shutdown still ends or kills the child after a protocol failure", async () => {
  ...
  await assert.rejects(transport.shutdown({ timeoutMs: 5 }));
  assert.equal(process.endObserved, true);
  assert.equal(process.killCalls, 1);
});

test("waits for stdout closure and drains a final frame after child exit", async () => {
  const process = createShutdownProcess({ exitOnEnd: false });
  const notifications = [];
  const transport = createJsonlTransport({ process });
  transport.onNotification((message) => notifications.push(message));
  process.stdin.once("finish", () => {
    process.exit(0, null, false);
    setImmediate(() => {
      process.send({ method: "turn/completed", params: { threadId: "thread-1", turn: { id: "turn-1" } } });
      process.stdout.end();
...
test("completes shutdown when stdout ended before exit without a process close event", async () => {
  const process = createShutdownProcess({ exitOnEnd: false });
  process.stdin.once("finish", () => {
    process.stdout.end();
    process.stderr.end();
...
test("allows an asynchronously killed real-process shape to close stdout before failing drain", async () => {
  const process = createShutdownProcess({ exitOnEnd: false, exitOnKill: false });
  process.kill = () => {
    process.killCalls += 1;
    setTimeout(() => process.exit(null, "SIGTERM"), 5);
    return true;
  };
  const transport = createJsonlTransport({ process });

  await transport.shutdown({ timeoutMs: 1 });
  assert.equal(process.killCalls, 1);
});
