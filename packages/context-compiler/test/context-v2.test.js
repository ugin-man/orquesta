...
test("Source Catalog recognizes both coordination and Placement task authorities", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "orquesta-context-task-authorities-"));
  try {
    const intent = taskIntent();
    const envelope = createTaskEnvelopeV2({ taskIntent: intent });
    const requirement = deriveContextRequirementV2({ taskIntent: intent, taskEnvelope: envelope });
    const catalog = buildSourceCatalogV2({
      workspaceRoot: root,
      taskIntent: intent,
      taskEnvelope: envelope,
    ...
    for (const reference of [".orquesta/state/tasks.json", ".orquesta/state/placement-tasks.json"]) {
      const record = catalog.find((item) => item.source_ref === reference);
      assert.equal(record?.source_type, "task_record", reference);
      assert.equal(record?.authority, "canonical", reference);
    }
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
