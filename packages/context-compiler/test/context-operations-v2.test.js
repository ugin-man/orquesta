...
test("coordination and Placement task authorities are both catalogued as canonical task records", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "orquesta-task-authorities-"));
  try {
    const state = path.join(root, ".orquesta", "state");
    fs.mkdirSync(state, { recursive: true });
    fs.writeFileSync(path.join(state, "tasks.json"), '{"version":1,"tasks":[]}\n', "utf8");
    fs.writeFileSync(path.join(state, "placement-tasks.json"), '{"schema_version":3,"project_id":"demo","revision":0,"tasks":[]}\n', "utf8");
    const catalog = refreshSourceCatalogV2({
      workspaceRoot: root,
      sourceRefs: [".orquesta/state/tasks.json", ".orquesta/state/placement-tasks.json"],
    });
    assert.deepEqual(
      catalog.records.map((record) => [record.source_ref, record.source_type, record.authority]),
      [
        [".orquesta/state/placement-tasks.json", "task_record", "canonical"],
        [".orquesta/state/tasks.json", "task_record", "canonical"],
      ],
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
