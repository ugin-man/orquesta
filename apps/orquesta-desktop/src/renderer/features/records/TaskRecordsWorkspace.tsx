...
function TaskRecordDetail({ task, agents, onClose }: { task: TaskUiModel; agents: AgentUiModel[]; onClose(): void }) {
  ...
  return (
    <aside className="task-record-detail" role="region" aria-label={`Task ${task.id} detail`}>
      <header>
...
export function TaskRecordsWorkspace({ tasks, agents, view, onViewChange }: {
  ...
  const copy = locale === 'ja'
    ? { scopes: 'タスクの表示範囲', incomplete: '未完了', complete: '完了', all: 'すべて', filters: 'タスクの絞り込み', search: 'タスクを検索', searchPlaceholder: 'IDまたはタイトルで検索', agent: '担当', allAgents: 'すべての担当', state: '状態', period: '更新期間', sort: '並び順', stateAll: 'すべての状態', active: '稼働中', waiting: '待機', user_wait: 'ユーザー待ち', review_wait: 'レビュー待ち', blocked: '停止中', failed: '失敗', periodAll: 'すべての期間', day: '24時間', week: '7日', month: '30日', newest: '更新が新しい順', oldest: '更新が古い順', id: 'ID順', results: '件を表示', empty: '条件に合うタスクはありません', outside: '選択中のタスクは現在の絞り込み条件の外にあります。', updated: '更新', owner: '担当', noSummary: '進捗の要約はありません' }
    : { scopes: 'Task scopes', incomplete: 'Incomplete', complete: 'Completed', all: 'All', filters: 'Task filters', search: 'Search tasks', searchPlaceholder: 'Search by ID or title', agent: 'Agent', allAgents: 'All agents', state: 'State', period: 'Updated', sort: 'Sort', stateAll: 'All states', active: 'Active', waiting: 'Waiting', user_wait: 'Waiting for user', review_wait: 'Review wait', blocked: 'Blocked', failed: 'Failed', periodAll: 'Any time', day: '24 hours', week: '7 days', month: '30 days', newest: 'Newest update', oldest: 'Oldest update', id: 'Task ID', results: 'shown', empty: 'No tasks match these filters.', outside: 'The selected task is outside the current filters.', updated: 'Updated', owner: 'Owner', noSummary: 'No progress summary' };
  const counts = {
  ...
  const scopes: Array<[TaskRecordScope, string]> = [['incomplete', copy.incomplete], ['complete', copy.complete], ['all', copy.all]];
  const stateOptions: Array<[TaskRecordStateGroup, string]> = [['all', copy.stateAll], ['active', copy.active], ['waiting', copy.waiting], ['user_wait', copy.user_wait], ['review_wait', copy.review_wait], ['blocked', copy.blocked], ['complete', copy.complete], ['failed', copy.failed]];

  return (
    <div className="task-records-workspace">
      <nav className="task-record-scopes" aria-label={copy.scopes}>
        {scopes.map(([scope, label]) => <button type="button" key={scope} aria-current={view.scope === scope ? 'page' : undefined} onClick={() => update({ scope })}><span>{label}</span><strong>{counts[scope]}</strong></button>)}
      </nav>
      <div className="task-record-filters" aria-label={copy.filters}>
        ...
        <label><span>{copy.state}</span><select aria-label={copy.state} value={view.stateGroup} onChange={(event) => update({ stateGroup: event.target.value as TaskRecordStateGroup })}>{stateOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      ...
      <div className={`task-record-layout${selected ? ' task-record-layout--detail-open' : ''}`} data-testid="task-record-layout">
        ...
        {selected ? <TaskRecordDetail task={selected} agents={agents} onClose={() => update({ selectedTaskId: null })} /> : null}
      </div>
    </div>
  );
}
