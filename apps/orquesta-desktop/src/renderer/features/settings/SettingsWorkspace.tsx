import { Activity, Bell, ChevronRight, FolderTree, GitBranch, Languages, MonitorCog, Play, RefreshCw, Settings2, TerminalSquare, type LucideIcon } from 'lucide-react';
import { useCallback, useEffect, useState } from 'react';
import type { RuntimeInfoUi } from '../../../contracts/bridge';
import type { ProjectStructureUiSnapshot, ProjectUiModel } from '../../../contracts/orquesta-ui';
import { formatDateTime } from '../../components/format';
import { useI18n } from '../i18n/I18nProvider';
import { ProjectStructureWorkspace } from './ProjectStructureWorkspace';

export type SettingsSection = 'display' | 'notifications' | 'codex' | 'startup' | 'structure' | 'status';
...
export function SettingsWorkspace({ project, projectStructure, reducedMotion, getRuntimeInfo, onOpenRoute, onOpenOperations, onStartHomeTutorial }: SettingsWorkspaceProps) {
  ...
  const copy = locale === 'ja' ? {
    navigation: '設定セクション', display: '表示', notifications: '通知', codex: 'Codex接続', startup: '起動とproject', structure: 'Project structure', status: '状態と診断',
    ...
    navigation: 'Settings sections', display: 'Display', notifications: 'Notifications', codex: 'Codex connection', startup: 'Startup & project', structure: 'Project structure', status: 'Status & diagnostics',
  ...
  const sections: Array<{ id: SettingsSection; label: string; icon: LucideIcon }> = [
    ...
    { id: 'structure', label: copy.structure, icon: FolderTree },
  ...
  return (
    <div className="settings-workspace">
      ...
      <section className="settings-content" aria-label={sections.find((item) => item.id === section)?.label}>
        ...
        {section === 'structure' ? <ProjectStructureWorkspace structure={projectStructure} /> : null}
