...
残った39.05 MiBはRenderer +33.68 MiBとGPU +6.55 MiBに集中し、Mainとnetworkはbaselineより微減しました。Chromiumが確保済みの描画／V8容量をWindowsへすぐ返さない挙動なので、メモリが完全に元へ戻ったとは記載しません。一方でtask継続、process増殖、DOM／listener／reachable heapの累積を示す証拠もありません。
