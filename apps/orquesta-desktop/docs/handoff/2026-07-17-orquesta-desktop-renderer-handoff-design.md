...
状態: 参考仕様（Renderer実装済み、desktop統合前）
実装状態: React Renderer Prototype完成
...
- 受け渡し内の参照: `public/reference/orquesta-desktop-home-approved.png`
- ChatGPTへ添付するときの推奨名: `orquesta-desktop-home-approved.png`

ローカルパスだけをChatGPTへ渡してはいけない。画像自体を添付する。

実装後の比較スクリーンショットは、ユーザー指示により今回の成果物には含めない。
